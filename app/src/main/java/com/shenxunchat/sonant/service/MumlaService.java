...
import com.shenxunchat.sonant.service.ipc.TalkBroadcastReceiver;
import com.shenxunchat.sonant.util.HtmlUtils;
...
public class MumlaService extends HumlaService implements
    ...
    private HumlaObserver mObserver = new HumlaObserver() {
        ...
        public void onConnected() {
            ...
            // Auto-join last channel for this server
            tryJoinLastChannel();
        }
        ...
        public void onUserJoinedChannel(IUser user, IChannel newChannel, IChannel oldChannel) {
            try {
                ...
                if (user != null && user.getSession() == selfSession && newChannel != null) {
                    saveLastChannel(newChannel.getId());
                }
    ...
    private static final String PREF_LAST_CHANNEL_PREFIX = "last_channel_";

    private void saveLastChannel(int channelId) {
        Server server = getTargetServer();
        if (server == null) return;
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        prefs.edit().putInt(PREF_LAST_CHANNEL_PREFIX + server.getId(), channelId).apply();
    }

    private void tryJoinLastChannel() {
        Server server = getTargetServer();
        if (server == null) return;
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        int lastChannel = prefs.getInt(PREF_LAST_CHANNEL_PREFIX + server.getId(), -1);
        if (lastChannel < 0) return;
        try {
            IHumlaSession session = HumlaSession();
            IChannel channel = session.getChannel(lastChannel);
            if (channel != null) {
                session.joinChannel(lastChannel);
            }
        } catch (Exception e) {
            Log.d(TAG, "Could not rejoin last channel: " + e);
        }
    }
