...
import com.shenxunchat.sonant.drawable.CircleDrawable;
...
public class ChannelListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements UserMenu.IUserLocalStateListener {
    ...
    private final boolean mShowCurrentOnly;
    ...
    public ChannelListAdapter(Context context, IHumlaService service, MumlaDatabase database,
                              ...
                              boolean showCurrentOnly, boolean showChannelUserCount) throws RemoteException {
        ...
        mShowCurrentOnly = showCurrentOnly;

        mRootChannels = new ArrayList<Integer>();
        if (showCurrentOnly) {
            IChannel currentChannel = mService.HumlaSession().getSessionChannel();
            if (currentChannel != null) {
                mRootChannels.add(currentChannel.getId());
            }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (node.isChannel()) {
            ...
            final boolean expandUsable = !mShowCurrentOnly &&
                    (channel.getSubchannels().size() > 0 ||
    ...
    public void updateChannels() {
        ...
        try {
            if (mShowCurrentOnly) {
                mRootChannels.clear();
                IChannel currentChannel = session.getSessionChannel();
                if (currentChannel != null) {
                    mRootChannels.add(currentChannel.getId());
                }
            }
    ...
    /**
     * Expands the channel path containing the supplied user so the user becomes visible.
     */
    public int revealUser(IUser user) {
        if (user == null || user.getChannel() == null) {
            return -1;
        }

        IChannel channel = user.getChannel();
        while (channel != null) {
            mExpandedChannels.put(channel.getId(), true);
            channel = channel.getParent();
        }

        updateChannels();
        notifyDataSetChanged();
        return getUserPosition(user.getSession());
    }
